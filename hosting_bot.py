# hosting_bot.py - Хостинг бот для Termux/Render
import telebot
...
def init_db():
    with get_db() as conn:
        ...
        ''')
        conn.execute('''
            INSERT OR IGNORE INTO promocodes (code, max_uses) VALUES 
            ('PREMIUM2024', 100),
            ('HOSTINGFREE', 50)
        ''')
        conn.commit()
...
def add_script(script_id, user_id, name, path, size):
    with get_db() as conn:
        conn.execute('INSERT INTO scripts (id, user_id, name, path, size) VALUES (?, ?, ?, ?, ?)',
                     (script_id, user_id, name, path, size))
        conn.commit()
...
def stop_all_scripts():
    for script in get_all_running_scripts():
        try:
            os.killpg(os.getpgid(script['pid']), signal.SIGTERM)
        except:
            try:
...
def get_main_menu(user_id):
    ...
    if user_id in ADMIN_IDS:
        if bot_status == "running":
            markup.add(InlineKeyboardButton("⏹ ОСТАНОВИТЬ БОТА", callback_data="admin_stop_bot"))
        else:
            markup.add(InlineKeyboardButton("▶️ ЗАПУСТИТЬ БОТА", callback_data="admin_start_bot"))
        ...
        markup.add(
            ...
            InlineKeyboardButton("🛑 Остановить всё", callback_data="admin_stop_all")
        )
...
def cmd_start(message):
    ...
    if bot_status == "stopped" and user_id not in ADMIN_IDS:
        bot.send_message(message.chat.id, "🔴 <b>Бот временно остановлен.</b>\nПопробуйте позже.", parse_mode='HTML')
        return
    
    if user_id in ADMIN_IDS or user['subscription'] == 'premium':
        max_scripts = "∞"
    else:
        max_scripts = FREE_MAX_SCRIPTS
    
    bot_status_text = "🟢 Работает" if bot_status == "running" else "🔴 Остановлен"
    
    text = (
        f"🚀 <b>Python Hosting Bot v{VERSION}</b>\n\n"
        f"Статус: {status}\n"
        f"Бот: {bot_status_text}\n"
        f"Скриптов: {count_user_scripts(user_id)}/{max_scripts}\n\n"
        f"Отправьте .py или .zip файл для запуска!"
    )
...
def admin_stats(call):
    ...
    text = (
        ...
        f"🆓 Бесплатных: {len(users) - premium_count}\n"
        f"📦 Всего скриптов: {len(all_scripts)}\n"
        f"🟢 Запущено: {running}\n"
        f"📡 Бот: {'🟢 Работает' if bot_status == 'running' else '🔴 Остановлен'}"
    )
...
def admin_users_list(call):
    ...
    for u in users[:20]:
        ...
        text += f"{status_icon} ID: <code>{u['user_id']}</code> | @{username}\n"
        markup.add(InlineKeyboardButton(
            f"📋 Скрипты {u['user_id']}",
            callback_data=f"admin_user_scripts:{u['user_id']}"
        ))
...
def admin_user_scripts(call):
    ...
    if not scripts:
        bot.send_message(call.message.chat.id, f"📭 У пользователя {user_id} нет скриптов")
        bot.answer_callback_query(call.id)
        return
    
    text = f"📋 <b>Скрипты пользователя {user_id} (@{user.get('username', 'Нет')}):</b>\n\n"
    ...
    for s in scripts:
        ...
        markup.add(
            InlineKeyboardButton(f"⏹ Стоп", callback_data=f"admin_stop_script:{s['id']}"),
            InlineKeyboardButton(f"📄 Логи", callback_data=f"admin_logs:{s['id']}"),
            InlineKeyboardButton(f"🗑 Удалить", callback_data=f"admin_delete_script:{s['id']}")
        )
    
    markup.add(InlineKeyboardButton("« Назад", callback_data="admin_users_list"))
...
def admin_view_logs(call):
    ...
    script_id = call.data.split(":")[1]
    log_path = os.path.join(LOGS_DIR, f"{script_id}.log")
    
    if os.path.exists(log_path):
        with open(log_path, 'rb') as f:
            bot.send_document(call.message.chat.id, f, caption=f"📄 Логи скрипта {script_id}")
        bot.answer_callback_query(call.id, "✅ Логи отправлены!")
    else:
        bot.answer_callback_query(call.id, "❌ Логов нет")

@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_stop_script:"))
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_delete_script:"))
def admin_delete_script(call):
    ...
    if script:
        delete_script(script_id, script['user_id'])
        script_dir = os.path.join(SCRIPTS_DIR, str(script['user_id']), script_id)
...
def admin_all_scripts(call):
    ...
    if not all_scripts:
        bot.send_message(call.message.chat.id, "📭 Нет скриптов")
        bot.answer_callback_query(call.id)
        return
    
    text = f"📋 <b>Все скрипты ({len(all_scripts)}):</b>\n\n"
    markup = InlineKeyboardMarkup(row_width=2)
    
    for s in all_scripts[:10]:
        status_icon = "🟢" if s['status'] == 'running' else "🔴"
        text += f"{status_icon} <b>{s['name']}</b>\n👤 ID: {s['user_id']}\nID: <code>{s['id']}</code>\n\n"
        
        markup.add(
            InlineKeyboardButton(f"📄 Логи {s['id']}", callback_data=f"admin_logs:{s['id']}"),
            InlineKeyboardButton(f"🗑 Удалить", callback_data=f"admin_delete_script:{s['id']}")
        )
...
def admin_stop_all(call):
    ...
    stop_all_scripts()
    bot.answer_callback_query(call.id, "🛑 Всё остановлено!")
    cmd_start(call.message)
...
def menu_upload(call):
    ...
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, "📦 Отправьте .py файл или ZIP-архив с вашим скриптом.")
...
def menu_premium(call):
    ...
    if call.from_user.id in ADMIN_IDS:
        bot.send_message(call.message.chat.id, "👑 У вас вечный премиум!")
        return
    
    bot.send_invoice(
        chat_id=call.message.chat.id,
        title="💎 Премиум подписка",
        description="Безлимит скриптов, до 1 ГБ",
        payload="premium_sub",
        provider_token="",
        currency="XTR",
        prices=[{"label": "Премиум", "amount": PREMIUM_STARS_PRICE}]
    )
...
def menu_promo(call):
    ...
    if call.from_user.id in ADMIN_IDS:
        bot.send_message(call.message.chat.id, "👑 Админам промокоды не нужны!")
        return
...
def menu_help(call):
    ...
    help_text = (
        "📚 <b>Помощь</b>\n\n"
        "<b>Кнопки меню:</b>\n"
        "📦 Запустить скрипт - загрузить .py или .zip\n"
        "📋 Мои скрипты - управление скриптами\n"
        "💎 Премиум - купить премиум\n"
        "🎁 Промокод - активировать промокод\n\n"
        "<b>Бесплатные промокоды:</b>\n"
        "PREMIUM2024\n"
        "HOSTINGFREE"
    )
...
def script_action(call):
    ...
    if action == "stop":
        ...
        if os.path.exists(log_path):
            with open(log_path, 'rb') as f:
                bot.send_document(call.message.chat.id, f, caption=f"📄 Логи {script_id}")
...
def show_scripts(message, edit=False):
    ...
    for s in scripts:
        ...
        markup.add(
            InlineKeyboardButton(f"⏹ Стоп {s['id']}", callback_data=f"script:stop:{s['id']}"),
            InlineKeyboardButton(f"📄 Логи {s['id']}", callback_data=f"script:logs:{s['id']}"),
            InlineKeyboardButton(f"🗑 Удалить {s['id']}", callback_data=f"script:delete:{s['id']}")
        )
...
def handle_document(message):
    if bot_status == "stopped" and message.from_user.id not in ADMIN_IDS:
        bot.reply_to(message, "🔴 Бот временно остановлен.")
        return
    ...
    if not check_user_limits(user_id):
        bot.reply_to(message, "❌ Лимит скриптов.")
        return
    ...
    if file_name.endswith('.zip'):
        ...
        if not ok:
            bot.reply_to(message, f"❌ Ошибка архива: {msg}")
            cleanup_temp(user_id)
            return

        py_files = find_py_files(extract_to)
        if not py_files:
            bot.reply_to(message, "❌ Нет .py файлов")
            cleanup_temp(user_id)
            return
        ...
        finish_script(user_id, temp_path, file_name)
...
def finish_script(user_id, script_path, original_filename):
    ...
    if error:
        bot.send_message(user_id, f"❌ Ошибка запуска: {error}")
        return

    update_script_status(script_id, 'running', pid)
    bot.send_message(user_id, f"✅ <b>Скрипт запущен!</b>\n\nID: <code>{script_id}</code>\nPID: <code>{pid}</code>", parse_mode='HTML')
    cleanup_temp(user_id)
...
def payment_success(message):
    ...
    bot.send_message(message.chat.id, "🌟 <b>Премиум активирован!</b>", parse_mode='HTML')
...
def monitor():
    while True:
        try:
            for script in get_all_running_scripts():
                if not is_process_alive(script['pid']):
                    update_script_status(script['id'], 'stopped')
        except:
            pass
...
if __name__ == '__main__':
    ...
    # Запуск с автоматическим восстановлением при ошибках
    while True:
        try:
